 ...
 *  column it stands in. */
export function MediaCopyFrame() {
  ...
  useEffect(() => {
    ...
    const read = () => {
      if (!document_.querySelector("[data-slot='media-copy-stepper']")) {
        return
      }
  ...
  return (
    ...
    >
      <FrameWindow width={width} height={height}>
        <iframe
          ref={frame}
          src="/specimens/media-copy"
          title={`The media section at ${String(width)}px`}
          style={{ width, height }}
          onLoad={() => {
